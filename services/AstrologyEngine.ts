...
export const AstrologyEngine = {
     ...
     */
    calculatePlanetaryPositions(date: Date, lat: number, lng: number): PlanetaryPositions {
        ...
        const getBodySign = (body: string) => {
            ...
            // @ts-ignore - Ecliptic definition might vary by version, relying on vector input often
    ...
    calculateFiveElements(date: Date): FiveElementsDistribution {
        ...
        // Base distribution
        let dist = { Wood: 20, Fire: 20, Earth: 20, Metal: 20, Water: 20 };

        // Modify based on Season (Month)
        // Spring (Feb-Apr): Wood strong
        if (month >= 2 && month <= 4) { dist.Wood += 30; dist.Earth -= 10; }
        // Summer (May-Jul): Fire strong
        else if (month >= 5 && month <= 7) { dist.Fire += 30; dist.Metal -= 10; }
        // Autumn (Aug-Oct): Metal strong
        else if (month >= 8 && month <= 10) { dist.Metal += 30; dist.Wood -= 10; }
        // Winter (Nov-Jan): Water strong
        else { dist.Water += 30; dist.Fire -= 10; }
