...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndtaXBwamFhY2lzcGpzbHRqZm9mIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY5NTQ0MDIsImV4cCI6MjA4MjUzMDQwMn0.R97-dN_1KkwPcA9hgKyciMPMGGQnimIty9iy9PHWDkI';
...
async function testProductsAPI() {
    ...
    try {
        ...
        const hasOnlySelectedFields = data.every(p =>
            Object.keys(p).length === 3 &&
            'id' in p && 'title' in p && 'price' in p
        );
...
async function testShippingAPI() {
    ...
    try {
        ...
        if (hasRates) {
            const totalRates = data.reduce((sum, z) => sum + z.shipping_rates.length, 0);
