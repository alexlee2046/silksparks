...
async function navigateViaHeader(page: any, linkText: string) {
    await page.getByRole('button', { name: linkText }).first().click();
...
test.describe('首页测试', () => {
    test('应该正确渲染首页', async ({ page }) => {
        ...
        // 检查 CTA 按钮
        await expect(page.getByRole('button', { name: /Reveal My Chart|View My Chart/i })).toBeVisible();
    });
...
test.describe('商城页面测试', () => {
    ...
    test('应该显示筛选选项', async ({ page }) => {
        await expect(page.getByRole('button', { name: /Intent/i }).first()).toBeVisible();
...
test.describe('UI 组件测试', () => {
    ...
    test('GlowButton 组件应该可点击', async ({ page }) => {
        ...
        const button = page.getByRole('button', { name: /Reveal My Chart|View My Chart/i });
