...
async function testTableStructure() {
    ...
    const requiredTables = [
        'profiles', 'system_settings', 'currencies', 'shipping_zones',
        'shipping_rates', 'products', 'experts', 'consultations',
        'orders', 'order_items', 'archives'
    ];
...
async function testDataIntegrity() {
    ...
    try {
        ...
        if (defaultCurrency) {
            log('PASS', '默认货币设置', `${defaultCurrency.code} - ${defaultCurrency.name}`);
        } else {
...
async function testCRUDOperations() {
    ...
    if (testProductId) {
        try {
            const res = await client.query('SELECT * FROM products WHERE id = $1', [testProductId]);
    ...
    if (testProductId) {
      ...
      `, [testProductId]);
            ...
            const res = await client.query('SELECT price FROM products WHERE id = $1', [testProductId]);
...
async function testQueryPerformance() {
    ...
    const queries = [
        { name: '产品列表', sql: 'SELECT * FROM products ORDER BY created_at DESC' },
        { name: '专家列表', sql: 'SELECT * FROM experts WHERE is_online = true ORDER BY rating DESC' },
        { name: '货币列表', sql: 'SELECT * FROM currencies ORDER BY id' },
        { name: '运费区域联表', sql: 'SELECT sz.*, sr.name as rate_name, sr.price FROM shipping_zones sz LEFT JOIN shipping_rates sr ON sz.id = sr.zone_id' }
    ];
