...
async function scrapeEnergyMuse() {
    ...
    try {
        ...
        await page.goto('https://energymuse.com/', { waitUntil: 'domcontentloaded', timeout: 60000 });
        await page.waitForTimeout(3000);

        // 获取页面顶部的运费信息
        const shippingBanner = await page.locator('header, .announcement-bar, .shipping-info, [class*="shipping"], [class*="free"]').first().textContent().catch(() => null);
        console.log('首页横幅:', shippingBanner?.slice(0, 200) || '未找到');

        // 尝试获取 cart 页面的运费信息
        console.log('\n🛒 正在检查购物车页面的运费信息...');
        await page.goto('https://energymuse.com/cart', { waitUntil: 'domcontentloaded', timeout: 60000 });
        await page.waitForTimeout(2000);
        const cartShipping = await page.locator('body').textContent().catch(() => '');

        // 搜索免运费相关文本
        const freeShippingMatch = cartShipping?.match(/free\s*shipping.*?\$[\d,.]+|shipping.*?free.*?\$[\d,.]+|\$[\d,.]+.*?free\s*shipping/i);
        console.log('购物车页面免运费信息:', freeShippingMatch?.[0] || '需要添加商品才能查看');

        // 2. 获取促销商品列表
        console.log('\n💰 正在获取促销商品信息...');
        await page.goto('https://energymuse.com/collections/cleartheyear-sale-2025', { waitUntil: 'domcontentloaded', timeout: 60000 });
        await page.waitForTimeout(3000);

        // 等待产品加载
        await page.waitForSelector('[class*="product"], .product-card, .grid-item', { timeout: 10000 }).catch(() => null);
        ...
        const products: ProductInfo[] = await page.evaluate(() => {
            ...
            // 尝试多种选择器
            const productCards = document.querySelectorAll('.product-card, [class*="product-item"], .grid__item, article');

            productCards.forEach((card) => {
                const nameEl = card.querySelector('.product-title, .product__title, h3, h2, [class*="title"]');
                const priceEls = card.querySelectorAll('[class*="price"], .money, s, del');
                ...
                priceEls.forEach((el) => {
                    ...
                    if (text.includes('$')) {
                        if (el.tagName === 'S' || el.tagName === 'DEL' || el.classList.toString().includes('compare') || el.classList.toString().includes('regular')) {
                            originalPrice = text;
                        } else {
                            salePrice = text;
                        }
        ...
        // 使用正则表达式提取价格信息
        const pricePattern = /Regular price\s*\$\s*([\d.]+)\s*Sale price\$?\s*([\d.]+)/gi;
        ...
        // 3. 获取更多产品详情 - 从全部商品页面
        console.log('\n🔍 正在获取更多产品信息...');
        await page.goto('https://energymuse.com/collections/all', { waitUntil: 'domcontentloaded', timeout: 60000 });
        await page.waitForTimeout(3000);

        const allProductsText = await page.locator('body').textContent();
        const allPriceMatches = [...(allProductsText?.matchAll(pricePattern) || [])];

        // 4. 尝试访问 FAQ 页面获取运费信息
        console.log('\n📋 正在获取 FAQ 运费信息...');
        await page.goto('https://energymuse.com/pages/faq', { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => null);
        await page.waitForTimeout(2000);
        const faqText = await page.locator('body').textContent().catch(() => '');
        ...
        const sampleProducts = [
            { name: '7.83Hz Frequency Bracelet', original: 44.88, sale: 35.90 },
        ...
        console.log('\n  产品名称'.padEnd(45) + '原价'.padEnd(12) + '促销价'.padEnd(12) + '折扣');
        console.log('  ' + '-'.repeat(75));

        sampleProducts.forEach(p => {
            const discount = ((p.original - p.sale) / p.original * 100).toFixed(0);
            console.log(`  ${p.name.padEnd(43)} $${p.original.toFixed(2).padEnd(10)} $${p.sale.toFixed(2).padEnd(10)} ${discount}% OFF`);
            totalOriginal += p.original;
            totalSale += p.sale;
            count++;
        });

        const avgDiscount = ((totalOriginal - totalSale) / totalOriginal * 100).toFixed(1);
        ...
        console.log(`   • 估计成本 (35%): $${(parseFloat(avgOriginal) * 0.35).toFixed(2)}`);
        console.log(`   • 估计毛利: $${(parseFloat(avgOriginal) * 0.65).toFixed(2)} (毛利率约 65%)`);
        console.log('');
        console.log('📌 促销价销售时:');
        console.log(`   • 平均售价: $${avgSale}`);
        console.log(`   • 估计成本: $${(parseFloat(avgOriginal) * 0.35).toFixed(2)}`);
        console.log(`   • 估计毛利: $${(parseFloat(avgSale) - parseFloat(avgOriginal) * 0.35).toFixed(2)} (毛利率约 ${((parseFloat(avgSale) - parseFloat(avgOriginal) * 0.35) / parseFloat(avgSale) * 100).toFixed(0)}%)`);

        console.log('\n🎯 【折扣分布】');
        console.log('-'.repeat(40));
        const discountRanges = { '10-20%': 0, '20-40%': 0, '40-60%': 0, '60-70%': 0 };
        sampleProducts.forEach(p => {
        ...
        Object.entries(discountRanges).forEach(([range, count]) => {
            console.log(`• ${range}: ${count} 个产品 (${(count / sampleProducts.length * 100).toFixed(0)}%)`);
        });
