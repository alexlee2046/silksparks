...
const SHELL_PATTERNS = [
  ...
  },
  {
    name: "coming-soon",
    regex:
      /['"]Coming Soon['"]|['"]即将推出['"]|['"]敬请期待['"]|['"]暂不可用['"]|['"]功能开发中['"]|['"]under development['"]|['"]not implemented['"]|['"]功能未实现['"]|['"]待开发['"]|['"]待实现['"]|['"]施工中['"]|['"]建设中['"]|['"]正在开发['"]|['"]开发中['"]|['"]筹备中['"]|['"]功能正在开发['"]|['"]Feature not available['"]|['"]Not available['"]|['"]TBD['"]|['"]TBA['"]|['"]WIP['"]/gi,
    severity: "medium",
    description: "占位符文字提示",
  },
...
class ShellDetector {
  ...
  async scanFile(filePath) {
    ...
    for (const pattern of SHELL_PATTERNS) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        const lineContent = (lines[lineNumber - 1] || match[0]).trim();
  ...
  generateReport() {
    ...
    const total = this.findings.length || 1; // 避免除以0
...
| 严重性 | 数量 | 比例 |
|--------|------|------|
| 🔴 高 | ${highCount} | ${((highCount / total) * 100).toFixed(1)}% |
| 🟡 中 | ${mediumCount} | ${((mediumCount / total) * 100).toFixed(1)}% |
| 🟢 低 | ${lowCount} | ${((lowCount / total) * 100).toFixed(1)}% |
